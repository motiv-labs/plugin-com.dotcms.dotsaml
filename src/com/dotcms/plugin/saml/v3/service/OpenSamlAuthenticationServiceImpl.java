...
public class OpenSamlAuthenticationServiceImpl implements SamlAuthenticationService {
	...
	public User resolveUser(final Assertion assertion, final IdpConfig idpConfig) {
		...
		try {
			...
			Logger.error(this, "An error occurred when loading user with ID '" + subjectNameID
					+ "'", e);
			user = null;
		}
		...
		if (user.isActive()) {
			...
			Logger.info(this, "User with ID '" + subjectNameID + "' is not active. No roles " +
					"were added.");
		}
